				...
				<?php
					...
					if ($_POST) {
						...
						if ($error){
							...
							$result = mysql_query("SELECT id FROM Links WHERE Link LIKE '%$name%'",$dbcon);
							...
							$sql = mysql_query("INSERT INTO Links (Key_words,Link) Values ('$key_words $user_link','$user_link')",$dbcon);
										 
							if (!$sql) {echo "Запрос не прошел.Попробуйте еще раз.";}
							else{
								echo "<h3 text-align='center'>Вы успешно добавили работу!</h3>";
							}
							mysql_close($dbcon);//закрытие MySQL.
							}	 
						}
					}
